...
impl<T> Grid<T> {
    ...
    pub fn new_from_grid<U: ExactSizeIterator<Item = T>, V: ExactSizeIterator<Item = U>>(
        in_values: V,
    ) -> Self {
    ...
    pub fn new_failable<U: Into<Box<[T]>>>(
        values: U,
        height: usize,
        width: usize,
    ) -> Result<Self, InvalidGridDimensions> {
        let t = values.into();
    ...
    pub fn new_from_grid_failable<
        U: ExactSizeIterator<Item = T>,
        V: ExactSizeIterator<Item = U>,
    >(
        in_values: V,
    ) -> Result<Self, InvalidGridDimensions> {
        let height = in_values.len();
...
impl NoiseRing {
    ...
    pub fn sample(&self, idx: f64) -> f64 {
        ...
        let idx2 =
            (((v + 1 as f64) * (self.values.len()) as f64) as usize).rem_euclid(self.values.len());
